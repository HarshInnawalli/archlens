...
```json metadata
{
  ...
  "tableOfContents": {"sections":[{"id":"safety-of-replication-when-master-has-persistence-turned-off","title":"Safety of replication when master has persistence turned off"},{"id":"how-redis-replication-works","title":"How Redis replication works"},{"id":"replication-id-explained","title":"Replication ID explained"},{"id":"diskless-replication","title":"Diskless replication"},{"id":"configuration","title":"Configuration"},{"id":"read-only-replica","title":"Read-only replica"},{"id":"setting-a-replica-to-authenticate-to-a-master","title":"Setting a replica to authenticate to a master"},{"id":"allow-writes-only-with-n-attached-replicas","title":"Allow writes only with N attached replicas"},{"id":"how-redis-replication-deals-with-expires-on-keys","title":"How Redis replication deals with expires on keys"},{"id":"configuring-replication-in-docker-and-nat","title":"Configuring replication in Docker and NAT"},{"id":"the-info-and-role-command","title":"The INFO and ROLE command"},{"id":"partial-sync-after-restarts-and-failovers","title":"Partial sync after restarts and failovers"},{"id":"maxmemory-on-replicas","title":"Maxmemory on replicas"}]}
...
of the Redis persistence. However, [WAIT](https://redis.io/docs/latest/commands/wait) dramatically reduces the probability of losing a write after a failure event to specific hard-to-trigger failure modes.
...
Read-only replicas will reject all write commands, so that it is not possible to write to a replica because of a mistake. This does not mean that the feature is intended to expose a replica instance to the internet or more generally to a network where untrusted clients exist, because administrative commands like [`DEBUG`](/commands/debug) or [`CONFIG`](/commands/config) are still enabled. The [Security](https://redis.io/docs/latest/operate/oss_and_stack/management/security/) page describes how to secure a Redis instance.
